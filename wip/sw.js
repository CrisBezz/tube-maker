...
self.addEventListener('activate',(event)=>event.waitUntil(caches.keys().then((keys)=>Promise.all(keys.filter((key)=>key!==CACHE && key.startsWith('mesh-line-replacer-v0.8.6-wip')).map((key)=>caches.delete(key)))).then(()=>self.clients.claim())));
