const CACHE='mesh-line-replacer-v0.9.0-wip.3';
const APP_SHELL=[
  ...
  './app-v087-static/grouped-instances.txt','./app-v088-static/view-modes.txt','./app-v089-static/instance-density.txt','./app-v089-static/density-v2-override.txt','./app-v090-static/compact-ui.txt','./app-v090-static/file-menu.txt','./app-v090-static/panel-cleanup.txt'
];
self.addEventListener('install',(event)=>event.waitUntil(caches.open(CACHE).then((cache)=>cache.addAll(APP_SHELL)).then(()=>self.skipWaiting())));
self.addEventListener('activate',(event)=>event.waitUntil(caches.keys().then((keys)=>Promise.all(keys.filter((key)=>key!==CACHE&&key.startsWith('mesh-line-replacer-')).map((key)=>caches.delete(key)))).then(()=>self.clients.claim())));
