# MeshUtilz WIP50 dependency and exclusion report

WIP50 consolidates the validated WIP49 four-primitive workflow into one clear Procedural Primitive UI. It does not change NOM data, scatter, placement, orientation, density, grouping, or any primitive adapter.
...
| `procedural-primitive-workflow-mu092wip50.txt` | Required importer and bridge | Owns Tube/Torus/Box/Cylinder parsing, cached-geometry bridging, and authoritative transform capture without any exporter or `Blob` interception. |
...
| `procedural-primitive-export-mu092wip50.txt` | Sole visible exporter | Validates, stages, and downloads through `window.__muW50`. |
...
- WIP21: mixed Torus importer and obsolete competing exporter; its required parser behavior is consolidated into WIP50 support.
- WIP22: superseded authority/export interception.
- WIP23: mixed bridge and export interception; its required bridge/capture behavior is consolidated into WIP50 support.
- WIP26 and WIP31: obsolete competing exporters.
- WIP33: diagnostic-only UI.
- WIP40–WIP49: rollback/reference exporters; no fallback is loaded.

## UI consolidation

- One `Procedural Primitive` section lists Tube, Torus, Box, and Cylinder support.
- One five-step workflow shows Load → Assign → Generate → Validate → Download.
- Primitive detection, cached-geometry information, validation, and download remain in one workflow.
- The legacy export anchor remains hidden and has no handler.
